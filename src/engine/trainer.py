...
class Trainer:
    ...
    def train_epoch(self, dataloader: torch.utils.data.DataLoader) -> tuple:
        ...
        for samples in tqdm(dataloader, desc='Training', leave=False):
            inputs, labels = samples['inputs'].to(self.device), samples['attributes']
            ...
            for task in self.tasks:
                ...
                if criterion.num_classes == 11:
                    task_label = (labels[task].to(self.device) - 1).long()
                elif criterion.num_classes == 1:
                    task_label = labels[task].to(self.device).float()
                ...
                with torch.no_grad():
                    ...
                    if criterion.num_classes == 1:
                        predicted_classes = (torch.sigmoid(task_output) > 0.5).float().squeeze(1)
                    else:
